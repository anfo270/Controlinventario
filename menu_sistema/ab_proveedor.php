    ...
    <div class="contenedor">
        ...
        <h1 style="color: #00047F">Agregar proveedor</h1>
            <form method="POST" action="../Config/alta_sistema.php"><center>
                ...
                <input type="text" name="nf" class="boxtext" placeholder="Nombre de proveedor"><br><br>
        ...
        <h1 style="color: #00047F">Eliminar proveedor</h1>
            <form method="POST" action="../Config/baja_sistema.php"><center>
                <input name="tipoAB" value="ac9a089c" hidden>
                <?php
                $res = $conexion->prepare("SELECT * FROM proveedor") or die(print($conexion->errorInfo()));
                $res->execute();

                if ($res->rowCount() > 0){
                    echo "<select type='text' name='nf' class='boxtext' placeholder='Elige proveedor'>";
                    while($row = $res->fetch()){
